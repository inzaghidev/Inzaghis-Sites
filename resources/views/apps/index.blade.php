    ...
    foreach ($apps as $app) {
      ...
      <div class="col-lg-3 col-md-4 col-sm-6 mb-4">
          <div class="card card-description h-100 border-success shadow">
              <a href="{{ $app->link }}"><img class="card-img-top card-img-apps" src="{{ $app->imgSrc }}" alt="{{ $app->name }}"></a>
              <div class="card-body card-description-body">
                  <div class="card-description">
                      <h4 class="card-name card-title page-apps-card"><a>{{ $app->name }}</a></h4>
                      <p class="card-apps-text card-text">{{ $app->description }}</p>
                  </div>
                  <button type="button" class="btn btn-primary btn-md apps-button"><a href="{{ $app->link }}" class="button-link">Click here</a></button>
              </div>
          </div>
      </div>
      <?php
